...
import { ChangeFullName, FetchFullNameRules } from "@/api/routes/accounts";
...
import { changeFullName } from "@/business/redux/features/user/userSlice";
import { validateField } from "@/utils/validation";
import { RootStyles } from "@/screens/RootStack/styles";
import { useFormik } from "formik";

const EditProfileFullName = ({
  navigation,
  route,
}: EditProfileStackScreenProps<"EditProfileFullName">) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      mutation.mutate(values.fullName.trim());
    },
  });

  // MUTATIONS
  const mutation = useMutation(
    (fullName: string) => ChangeFullName(fullName, tokenApi),
    {
      onSuccess(data) {
        if (user) {
    ...
    <Container dismissKeyboardEnabled>
      <View style={styles.container}>
        <FormContainer>
          <FormControl>
            <FormControl.Label>
              <Text style={EditProfileStyles.label}>Name</Text>
            </FormControl.Label>
            <Input
              ref={inputRef}
